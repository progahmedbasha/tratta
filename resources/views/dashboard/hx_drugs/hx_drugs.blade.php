...
<div class="card">
    ...
    <div class="card-body p-3">
        <form action="{{route('hx_drugs.update', $hx_drug->id)}}" method="post" enctype="multipart/form-data">
            @csrf
            @method('patch')
            <div class="row">
                <div class="col-md-4">
            ...
            <div class="row">
                <div class="col-md-8">
                    <textarea class="form-control" style="background-color: {{ $hx_drug->interactionSeverity->color }}; color:azure; " placeholder="Note" name="note">{{ $hx_drug->note }}</textarea>
                </div>
                <div class="col">
                    <select class="form-control" name="interaction_severity_id" required />
                    <option value="">Sevrity</option>
                    @foreach ($interaction_severities as $interaction_severity)
                    <option value="{{$interaction_severity->id}}" {{($hx_drug->interaction_severity_id
